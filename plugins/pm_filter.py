        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"♦️{get_size(file.file_size)}🔸 {file.file_name}", callback_data=f'files#{nxreq}#{file.file_id}'
                ),
            ]
            for file in files
        ]
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):        
        ...
        buttons = [[
            ...
            InlineKeyboardButton("📢 𝚄𝙿𝙳𝙰𝚃𝙴𝚂 📢", url="https://t.me/pushpan2")
            ],[
            InlineKeyboardButton("🎥 ᴍᴏᴠɪᴇ ᴜᴘᴅᴀᴛᴇs", url="https://t.me/pushpan2")
            ],[      
        ...
        buttons = [[
            InlineKeyboardButton('SOURCE CODE', url='https://t.me/vipzzk')
            ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if settings["button"]:
        btn = [
            [
                InlineKeyboardButton(
                    text=f"♦️{get_size(file.file_size)}🔸 {file.file_name}", callback_data=f'{pre}#{req}#{file.file_id}'
                ),
            ]
            for file in files
        ]
